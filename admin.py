...
def dashboard():
    ...
    approved_devices = Device.query.filter_by(status="Approved").order_by(Device.user_id, Device.id).all()
...
# ------------------------------------------------------------
# NUKE DEVICE (delete device + logs only)
# ------------------------------------------------------------
@admin_bp.route("/device/nuke/<int:device_id>", methods=["POST"])
@login_required
def nuke_device(device_id):
    if not current_user.is_admin:
        return "Forbidden", 403

    ActivityLog.query.filter_by(device_id=device_id).delete()
    ...
    flash("Device and logs erased permanently.", "danger")
    return redirect(url_for("admin.dashboard"))
